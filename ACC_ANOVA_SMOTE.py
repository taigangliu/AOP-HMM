...
def main(name,feature_extraction,feature_selection,standard,G,dimension,over_sampling):
    ...
    # with open('ob1.txt','w') as f:
    #     for i in range(len(y_train)):
    #         f.writelines(str(x_train_zip[i][0])+" "+str(y_train[i])+" "+str(x_train_zip[i][0] >= 1552)+'\n')
    ...
    # with open('ob1.5.txt','w') as f:
    #     for i in range(len(dataset)):
    #         f.writelines(str(dataset[i][0][0])+" "+str(dataset[i][1])+" "+str(dataset[i][0][0] >= 1552)+"\n")
    ...
    # with open('ob2.txt','w') as f:
    #     for i in range(len(y_train2)):
    #         f.writelines(str(shuffle_index[i])+" "+str(y_train2[i])+" "+str(shuffle_index[i] >= 1552)+"\n")
        ...
        for k in range(f1,dimension+1,5):
            ...
            for i in range(3, -15-1, -2):
                for j in range(-3, 15+1, 2):
                    ...
                    print("y_true:"+str(Counter(y_true)))
                    print("y_true2:"+str(Counter(y_true2)))
                    score = accuracy_score(y_true2, y_pred)
                    # print(score)
        ...
        for k in range(f1,dimension+1,5):
            ...
            for i in range(3, -15-1, -2):
                for j in range(-3, 15+1, 2):
                    ...
                    clf = SVC(C=c, gamma=g, kernel='rbf', probability=True, random_state=7)
                    # train loo
                    score1 = cross_val_score(clf,train_x,train_y,cv=KF,scoring='accuracy',n_jobs=-1).mean()
                    # test
...
main('Feng',['ACC'],'ANOVA','Minmax',11,350,'SMOTE')
main('Zhang',['ACC'],'ANOVA','Minmax',11,350,'SMOTE')