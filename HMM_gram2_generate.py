...
def getfeature_gram2():
    feature, target = gethmmdata('Feng')
    strlen = len(feature)
    file = open('./feature_data/Feng/Fenggram2.csv', 'a', newline='') 
    content = csv.writer(file, dialect='excel')  
    for i in range(0, strlen):
        content.writerow(feature[i])
    feature, target = gethmmdata('ZhangTrain')
    strlen = len(feature)
    file = open('./feature_data/ZhangTrain/ZhangTraingram2.csv', 'a', newline='')  
    content = csv.writer(file, dialect='excel')  
    for i in range(0,strlen):
        content.writerow(feature[i])
    feature, target = gethmmdata('ZhangTest')
    strlen = len(feature)
    file = open('./feature_data/ZhangTest/ZhangTestgram2.csv', 'a', newline='')  
    content = csv.writer(file, dialect='excel')  
    for i in range(0, strlen):
        content.writerow(feature[i])
