...
def getfeature_gram1():
    ...
    strlen = len(feature)
    file = open('./feature_data/Feng/Fengseqgram1.csv', 'a', newline='')  
    content = csv.writer(file, dialect='excel')  
    for i in range(0, strlen):
        content.writerow(feature[i])
    feature, target = getseqdata('ZhangTrain')
    strlen = len(feature)
    file = open('./feature_data/ZhangTrain/ZhangTrainseqgram1.csv', 'a', newline='') 
    content = csv.writer(file, dialect='excel') 
    for i in range(0,strlen):
        content.writerow(feature[i])
    feature, target = getseqdata('ZhangTest')
    strlen = len(feature)
    file = open('./feature_data/ZhangTest/ZhangTestseqgram1.csv', 'a', newline='')  
    content = csv.writer(file, dialect='excel')  
    for i in range(0, strlen):
        content.writerow(feature[i])
