...
def main(name,feature_extraction,feature_selection,standard,G,dimension,over_sampling):
    ...
    if feature_selection != '':
        ...
        if feature_selection == 'ANOVA':
            ...
            print("importances=\n", importances)
            feature_list = np.argsort(importances)[::-1]
            print("feature_list=\n", feature_list)
        elif feature_selection == 'XGB':
            forest = XGBClassifier(random_state=7)
            forest.fit(x_train,y_train)
            importances = forest.feature_importances_
            print("importances=\n", importances)
            feature_list = np.argsort(importances)[::-1]
...
main('Feng',['ACC'],'XGB','Minmax',11,350,'')
main('Zhang',['ACC'],'XGB','Minmax',11,350,'')
